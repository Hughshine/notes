...
“一致性”：一个句子集具有一致性，当且仅当它不能同时证明一个命题以及它的否定（即，要求一个句子集是自洽的）。
“可满足性”：句子的真值与它包含的命题符号的真值的关系（structural deduction）。【首先要定义真值指派】。公式集   是可满足的，当且仅当存在一个真值指派s满足   .
命题逻辑系统的完全性成立，等价于“假如一个公式集   一致，那么它可满足”。
 我觉得我得重头看一遍数理逻辑。
