...
\textbf{在同步语句只包含\texttt{lock}与\texttt{unlock}的语言上，验证Deadlock.}
	...
	\begin{center}
		...
		{Unblocked(pid, \sigma)\;\;\;\neg LockFree(res, \sigma)\;\;\;\textbf{let}\; l' = \sigma.ParRequest(pid).\;l'\not=\perp}  % 需要增加一条，删除对以前资源的依赖 ParRequest(pid\rightsquigarrow \perp)
		...
		{Unblocked(pid, \sigma)\;\;\; Own(pid, res, \sigma) \vee ResFree(res, \sigma)}  % 需要增加一条，删除对以前资源的依赖 ParRequest(pid\rightsquigarrow \perp)
		...
		{Unblocked(pid, \sigma)}
		{(pid' = \textbf{subproc}(pid), \sigma) \longrightarrow (\textbf{skip}, \sigma(Par(pid' \rightsquigarrow nextpid()), ParRequest(pid'\rightsquigarrow\perp)))}  % TODO: nextpid()
		$
		...
		{Unblocked(pid, \sigma)}  % TODO: 可以增加语义让所有资源释放
