...
(* 可以将验证过的代码提取到其他函数式语言中，编译后可以得到验证过了的程序*)
(* ================================================================= *)
...
Inductive nat : Type :=
  | O
  | S (n : nat).  (*S 和 一般方式定义的函数有差异，它并不代表一种计算，只是一个notation；只是我们可以赋予它一种意义。*)
...
(* reflexivity 可以自动 simpl. 额外添加它是为了看中间步骤.*)
...
Proof.
  ...
  rewrite <- H. (*箭头方向的意义：用H的左侧代替右侧/右侧代替左侧*)
  reflexivity.  Qed.
...
Theorem andb_false : forall a : bool,
  andb a false = false.
Proof. 
  intros a.
...
(* Fixpoint plus'' (n m : nat) : nat := plus' n m. 
This will report: not a truly recursive fixpoint.
*)
...
Theorem orb_true_elim2 :
  forall b, (orb true b) = true.
Proof. 
  (* intros b.  怎么会呢？？？true && b == true => b == true *)
  destruct b.
  - reflexivity.
  - reflexivity.  Qed.

(* Theorem and_true1_true2 :
  forall b, (and) *)

Theorem andb_trueres_true1_true2 :
  forall b, (andb true b) = true -> b = true.
Proof.
  intros b.
  simpl. 
  intros H.
...
Theorem andb_false_elim2 : 
  forall c, (false && c) = false.
Proof. intros c. simpl. reflexivity.  Qed.
...
Example test_bin_inc2:
  incr (B (B Z)) = A (A (B Z)).
Proof. simpl. reflexivity. Qed.

Example test_bin_to_net1: 
  bin_to_nat (A (A (A (B Z)))) =  S (S (S (S (S (S (S (S O))))))).
Proof. simpl. reflexivity. Qed.

Example test_bin_to_net2:
  bin_to_nat (B (B (B Z))) =  S (S (S (S (S (S (S O)))))).
Proof. simpl. reflexivity. Qed.

Example test_inc_and_bin2net:
  bin_to_nat (incr (B (B (B Z)))) = 1 + (bin_to_nat (B (B (B Z)))).
Proof. simpl. reflexivity. Qed.
(* Do not modify the following line: *)
