...
Notation "( x , y )" := (pair x y).  (*new notation can also be used in pattern matches*)
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with 
  | [] => []
  | x :: xs => if (oddb x) then x :: oddmembers xs else oddmembers xs end.

Example test_oddmembers:
  oddmembers [0;1;0;2;3;0;0] = [1;3].
  Proof. simpl. reflexivity.  Qed.

(* Definition 与 Fixpoint 的差别: recursion. 虽然题目本身给的是Definition，但应该一定要改成Fixpoint.*)
Fixpoint countoddmembers (l:natlist) : nat :=
  ...
  (* | x :: xs => if (oddb x) then (S (countoddmembers xs)) else countoddmembers xs end. *)
...
(* 这里怪怪的：不能直接对bag调用cons. 哦，这个Definition是有参数的，sum直接给了函数的类型，没有参数*)
...
Fixpoint member (v:nat) (s:bag) : bool :=
  match s with 
  | [] => false
  | x :: xs => if (v =? x) then true else (member v xs)
  end.
...
Theorem bag_theorem : forall v s, 
