...
Add LoadPath "D:\PersonalNote\plnotes-collection\FormalSemanticsOfProgLang\lf\lf\" as LF.
(* Print LoadPath. *)
...
(** In this chapter we continue our development of basic
    ...
    they manipulate) 多态函数
     and _higher-order functions_ (treating functions
    as data). 高阶函数 
    We begin with polymorphism. *)
...
Inductive grumble (X:Type) : Type := (**每个构造器需要一个类型参数**)
  | d (m : mumble)
  | e (x : X).
...
(*完全使用 implicit (type) arguments: a step too far.*)
...
Proof.
  ...
  - simpl. 
    rewrite -> Ihl1'. (* (rev l2 ++ rev l1') ++ [x] = rev l2 ++ rev l1' ++ [x] *)
    rewrite -> app_assoc. 
    reflexivity.
  Qed.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  ...
  (* filter (fun n => ((evenb n) && (negb (leb 7 n)))) l. *)
